...
double pyramid_surface(double b, double h) { return b * b + 2.0 * b * sqrt(b * b / 4.0 + h * h); }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Volume of a cube of width/height/depth " << x << ": " 
            ...
            << "\nVolume of a right rectangular pyramid with equal base width and length "
            << b << " and height " << h << ": " << pyramid_volume(b, h) 
            << ", surface area: " << pyramid_surface(b, h) << "\nVolume of a right cylinder "
            << " of radius " << r << " and height " << h << ": " << cylinder_volume(r, h)
            << ", surface area: " << cylinder_surface(r, h)
            << "\nVolume of a sphere of radius " << r << ": " << sphere_volume(r)
            << ", surface area: " << sphere_surface(r) << '\n';
