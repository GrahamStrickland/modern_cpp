...
public:
  ...
  vector(const vector &that) : vector(that.size()) { copy(that); }
...
template <typename Value> struct std::formatter<vector<Value>> {
  template <typename FormatContext>
  auto format(const vector<Value> &v, FormatContext &ctx) const {
    auto &&out = ctx.out();
    std::format_to(out, value_format, v[0]);
    for (int i = 1; i < v.size(); ++i)
      std::format_to(out, ", " + value_format, v[i]);
...
int main(int argc, char *argv[]) {
  vector<double> v{1.394, 1e9, 1.0 / 3.0, 1e-20};

  std::print("v with empty format = {}.\n", v);
  std::print("v with f = {:f}.\n", v);
  std::print("v witdh 9, 4 digits = {:9.4f}.\n", v);
  std::print("v scient. = {:ec}.\n", v);
